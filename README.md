...
Нужны Go 1.25.13+ и PostgreSQL 18 с отдельной базой для приложения. Inventory создаёт таблицы и демо-данные при старте. `SEED_DEMO=false` отключает заполнение каталога.
...
Сейчас проект состоит из трёх работающих сервисов с gRPC-контрактами и health checks. Внешний Gateway предоставляет REST API, SSE-поток, Prometheus-метрики, сквозные request ID и веб-интерфейс, Booking управляет сценарием бронирования, а Inventory хранит каталог и состояние мест в PostgreSQL.
