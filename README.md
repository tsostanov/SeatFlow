...
Страница показывает схему мест, карточку билета и оставшееся время. Бронь можно восстановить по ID, а незавершённый запрос переживает перезагрузку вкладки: браузер повторяет его с первоначальным ключом.
...
Логика восстановления запросов проверяется отдельно, без браузера и внешних npm-пакетов. Нужен Node.js 22+ (только для этих тестов; приложение запускается без Node.js):
