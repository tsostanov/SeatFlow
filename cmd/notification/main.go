...
func run() error {
	...
	defer db.Close()
	svc, err := notification.New(startup, db)
	if err != nil {
		return err
	}
	return platform.ServeGRPC(ctx, platform.Env("GRPC_ADDR", "127.0.0.1:50054"), func(s *grpc.Server) { pb.RegisterNotificationServiceServer(s, svc) }, db.Ping)
}
