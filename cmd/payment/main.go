...
func main() {
	if err := run(); err != nil {
		slog.Error("payment stopped", "error", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	defer db.Close()
	svc, err := payment.New(startup, db)
	if err != nil {
		return err
	}
	return platform.ServeGRPC(ctx, platform.Env("GRPC_ADDR", "127.0.0.1:50053"), func(s *grpc.Server) { pb.RegisterPaymentServiceServer(s, svc) }, db.Ping)
}
